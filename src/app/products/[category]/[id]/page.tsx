...
import { ChevronRight, ShoppingCart, Heart, Check, Star, Truck, Shield, ArrowLeft, PawPrint, Bird } from "lucide-react"
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-20">
          ...
          >
            ...
            <div className="absolute top-4 left-4 flex gap-2">
              {product.isNew && <span className="px-3 py-1.5 bg-vet-500 text-white text-xs font-bold rounded-full">NEW</span>}
              {product.isSale && <span className="px-3 py-1.5 bg-red-500 text-white text-xs font-bold rounded-full">SALE</span>}
            </div>
          </motion.div>
          ...
          <AnimatedSection direction="right">
            ...
            <div className="flex items-baseline gap-3 mb-6">
              ...
              {product.isSale && (
                <span className="text-lg text-neutral-400 line-through">{product.price} DZD</span>
              )}
              {product.isSale && (
            ...
            <div className="flex items-center gap-3 mb-8">
              <div className="flex items-center gap-1.5 text-sm text-neutral-600">
                ...
                Free delivery on orders +5000 DZD
              </div>
              <div className="flex items-center gap-1.5 text-sm text-neutral-600">
        ...
        {related.length > 0 && (
          <section>
            <AnimatedSection className="flex items-center justify-between mb-8">
              <div>
                <span className="text-brand-600 text-sm font-semibold tracking-widest uppercase">Related</span>
                <h2 className="text-2xl font-bold mt-1">More {product.category === "cats" ? "Cat" : product.category === "birds" ? "Bird" : ""} Products</h2>
              </div>
                ...
                >
                  <Link href={`/products/${p.category}/${p.id}`} className="group block bg-white rounded-2xl border border-neutral-100 overflow-hidden hover:shadow-lg transition-all duration-500">
                    <div className="relative aspect-square bg-gradient-to-br from-neutral-100 to-neutral-200 overflow-hidden">
                      ...
                      {p.isNew && <span className="absolute top-2 left-2 px-2 py-0.5 bg-vet-500 text-white text-[10px] font-bold rounded-full">NEW</span>}
                    </div>
                    <div className="p-4">
