...
export default function ShippingPage() {
  ...
  return (
    <div className="bg-[#0F1913] text-white pb-20 min-h-screen" dir={dir}>

      {/* ═══════════════════════════════
          HERO SECTION
      ═══════════════════════════════ */}
      <section className="relative overflow-hidden pt-28 sm:pt-36 pb-14 lg:pb-20 bg-gradient-to-b from-[#13111C] via-[#1A1A2E] to-[#0D0B14] text-white border-b border-white/10">
        {/* Glow & Paw Decor */}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <AnimatedSection>
              {/* Badge */}
              <div className="inline-flex items-center gap-2 bg-[#E3602D]/15 border border-[#E3602D]/40 text-[#F1C290] rounded-full px-4 py-1.5 text-sm font-medium mb-5 shadow-sm">
                <Truck className="w-4 h-4 text-[#F5851F]" />
                {t.shipping.heroBadge}
              </div>

              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white leading-[1.1] mb-5 drop-shadow-lg">
                {t.shipping.title}
              </h1>
              <p className="text-base sm:text-lg text-emerald-100/70 max-w-md leading-relaxed mb-8">
                {t.shipping.subtitle}
              </p>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <span className="inline-flex items-center gap-2 text-sm text-emerald-100/60 font-medium">
                ...
                <div className="rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-8 shadow-2xl">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={i} className="text-center rounded-xl bg-white/5 border border-white/10 py-3 px-2">
                        <div className="flex justify-center text-[#F1C290] mb-1">{s.icon}</div>
                        <div className="text-base font-bold text-white">{s.v}</div>
                        <div className="text-[10px] text-emerald-100/60 leading-tight mt-0.5">{s.l}</div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 pt-10">
        ...
        <AnimatedSection>
          <div className="rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-8 lg:p-10 shadow-2xl">
            <div className="text-center mb-10">
              <h2 className="text-2xl sm:text-3xl font-black text-white">{t.shipping.stepsTitle}</h2>
              <p className="text-emerald-100/60 mt-2 text-sm">{t.shipping.stepsSubtitle}</p>
            </div>
              ...
              {steps.map((s, i) => {
                ...
                return (
                  <div key={i} className="relative group">
                    <div className="h-full rounded-2xl bg-black/30 border border-white/10 p-6 hover:border-[#E3602D]/40 transition-colors duration-300">
                      <span className="absolute -top-3 -start-3 w-8 h-8 rounded-full bg-[#E3602D] text-white text-sm font-bold flex items-center justify-center shadow-lg shadow-[#E3602D]/30">
                        {i + 1}
                      </span>
                      <div className="w-12 h-12 rounded-xl bg-[#E3602D]/15 flex items-center justify-center text-[#F1C290] mb-4">
                        <Icon className="w-6 h-6" />
                      </div>
                      <h3 className="text-base font-bold text-white mb-2">{s.title}</h3>
                      <p className="text-xs sm:text-sm text-emerald-100/70 leading-relaxed">{s.desc}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8">
            <AnimatedSection delay={100} className="h-full">
              <div className="h-full rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-8 lg:p-10 shadow-2xl">
                <div className="mb-8">
                  <h2 className="text-2xl font-black text-white">{t.shipping.pricingTitle}</h2>
                  <p className="text-emerald-100/60 mt-2 mb-6 text-sm">{t.shipping.pricingSubtitle}</p>
                    ...
                    return (
                      <div
                        ...
                        className={`relative rounded-2xl p-6 border transition-all duration-300 ${
                          featured
                            ? "bg-gradient-to-b from-[#E3602D]/20 to-black/30 border-[#E3602D]/50 shadow-inner"
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <MapPin className={`w-4 h-4 ${featured ? "text-[#F1C290]" : "text-white/60"}`} />
                        </div>

                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-emerald-100/60">{t.shipping.deliveryTableHeaders.eta}</span>
                            <span className="font-bold text-sm text-white">{r.eta}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-emerald-100/60">{t.shipping.deliveryTableHeaders.fee}</span>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <AnimatedSection delay={200} className="flex-1">
              <div className="h-full rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-8 flex flex-col justify-center shadow-2xl">
                <div className="w-12 h-12 rounded-xl bg-[#E3602D]/15 flex items-center justify-center text-[#F1C290] mb-4">
                  <CreditCard className="w-6 h-6" />
                </div>
                <h3 className="text-lg font-bold text-white mb-2">{t.shipping.codTitle}</h3>
                <p className="text-xs sm:text-sm text-emerald-100/70 leading-relaxed">{t.shipping.codDesc}</p>
              </div>
            </AnimatedSection>
            ...
            <AnimatedSection delay={300} className="flex-1">
              <div className="h-full rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-8 flex flex-col justify-center shadow-2xl">
                ...
                <p className="text-xs sm:text-sm text-emerald-100/70 leading-relaxed">{t.shipping.returnsDesc}</p>
              </div>
            </AnimatedSection>
          </div>
        </div>
      ...
      {t.shipping.sections && t.shipping.sections.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
          <AnimatedSection>
            <div className="rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-8 lg:p-10 space-y-8 shadow-2xl">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <AnimatedSection>
          <div className="relative overflow-hidden rounded-3xl border border-[#E3602D]/40 bg-gradient-to-br from-white/10 via-black/40 to-black/60 p-10 lg:p-14 text-center shadow-2xl backdrop-blur-xl">
            {/* Glow effects */}
            <div className="absolute top-1/2 left-1/4 w-64 h-64 bg-[#E3602D] rounded-full opacity-15 blur-3xl -translate-y-1/2 pointer-events-none" />
            <div className="absolute top-1/2 right-1/4 w-64 h-64 bg-[#F1C290] rounded-full opacity-15 blur-3xl -translate-y-1/2 pointer-events-none" />

            <div className="relative z-10 flex flex-col items-center justify-center">
              <div className="inline-flex items-center gap-2 bg-[#E3602D]/20 border border-[#E3602D]/30 text-[#F1C290] rounded-full px-4 py-1.5 text-sm font-medium mb-5">
                <ShieldCheck className="w-4 h-4" />
                {t.shipping.subtitle}
              </div>
              <h2 className="text-2xl lg:text-3xl font-black text-white mb-3">{t.shipping.ctaTitle}</h2>
              <p className="text-emerald-100/70 max-w-md mx-auto mb-8 text-sm">{t.shipping.ctaDesc}</p>
              <Link
                href="/products"
